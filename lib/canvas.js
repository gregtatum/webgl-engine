...
var internals = {
	...
	processCanvas : function( config, state ) {
		...
		if( config.resize ) {
			...
			resizeHandler = () => internals.resizeCanvas( state.gl, config.canvas, config.container )
	...
	resizeCanvas : function( gl, canvas, container ) {

		var ratio = _.isNumber( window.devicePixelRatio ) ? window.devicePixelRatio : 1
...
module.exports = function( properties ) {
	...
	var state = {}
	state.gl = internals.getContext( config.canvas )
	state.resizeHandler = internals.processCanvas( config, state )
